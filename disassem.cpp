...
int main(int argc, char* argv[]){
    ...
    //loop through textrecord until all instructions or translated
    while(tRecords.length() > 1){
        bool isType2 = false;
        int opcode = stoi(tRecords.substr(0, 2), 0, 16) & 0b11111100;

        //check for type 2 instructions
        for(unsigned int i = 0; i < sizeof(ops2)/sizeof(ops2[0]); i++){
            if(opcode == ops2[i]){
                isType2 = true;
        ...
        for(unsigned int i = 0; i < sizeof(ops3)/sizeof(ops3[0]); i++){
            if(opcode == ops3[i]){
        ...
        //check for target address addressing
        string TAAM;
        ...
        //handle type 3 vs 4 object code displacement
        if(isType4){
